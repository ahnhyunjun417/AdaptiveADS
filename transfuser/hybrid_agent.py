...
def check_obb_intersection(obb1, obb2):
    ...
    return not (get_separating_plane(r_pos, obb1.rotation.get_forward_vector(), obb1, obb2) or
                ...
                get_separating_plane(r_pos, obb2.rotation.get_up_vector(), obb1, obb2) or
                get_separating_plane(r_pos, cross_product(obb1.rotation.get_forward_vector(),
                ...
                get_separating_plane(r_pos, cross_product(obb1.rotation.get_right_vector(),
                                                          obb2.rotation.get_forward_vector()), obb1, obb2) or
                get_separating_plane(r_pos, cross_product(obb1.rotation.get_right_vector(),
                                                          obb2.rotation.get_right_vector()), obb1, obb2) or
                get_separating_plane(r_pos, cross_product(obb1.rotation.get_right_vector(),
                                                          obb2.rotation.get_up_vector()), obb1, obb2) or
                get_separating_plane(r_pos, cross_product(obb1.rotation.get_up_vector(),
                                                          obb2.rotation.get_forward_vector()), obb1, obb2) or
                get_separating_plane(r_pos, cross_product(obb1.rotation.get_up_vector(),
                                                          obb2.rotation.get_right_vector()), obb1, obb2) or
                get_separating_plane(r_pos, cross_product(obb1.rotation.get_up_vector(),
...
def non_maximum_suppression(bounding_boxes, iou_threshold):
    filtered_boxes = []
    ...
    while len(confidences_indices) > 0:
        ...
        for idx2 in deepcopy(confidences_indices):
            if iou_bbs(current_bb, bounding_boxes[idx2, 0]) > iou_threshold:  # Remove BB from list
...
class HybridAgent(autonomous_agent.AutonomousAgent):
    def setup(self, path_to_conf_file, route_index=None):
        ...
        self.iou_threshold_nms = self.config.iou_treshold_nms
        ...
        for file in os.listdir(path_to_conf_file):
            if file.endswith(".pth"):
                ...
                net = LidarCenterNet(self.config, 'cuda', self.backbone, image_architecture, lidar_architecture,
                                     use_velocity)
                if self.config.sync_batch_norm:
                    net = torch.nn.SyncBatchNorm.convert_sync_batchnorm(net)
                    # Model was trained with Sync. Batch Norm.
                    # Need to convert it otherwise parameters will load incorrectly.
                state_dict = torch.load(os.path.join(path_to_conf_file, file), map_location='cuda:0')
                state_dict = {k[7:]: v for k, v in state_dict.items()}
                # Removes the .module coming from the Distributed Training.
                # Remove this if you want to evaluate a model trained without DDP.
    ...
    def sensors(self):
        ...
        if SAVE_PATH is not None:  # Debug camera for visualizations
            sensors.append({
                'type': 'sensor.camera.rgb',
                'x': -4.5, 'y': 0.0, 'z': 2.3,
    ...
    def tick(self, input_data):
        ...
        for pos in ['left', 'front', 'right']:
            ...
            rgb_pos = scale_crop(Image.fromarray(rgb_pos), self.config.scale, self.config.img_width,
                                 self.config.img_width, self.config.img_resolution[0],
        ...
        if np.isnan(compass):  # CARLA 0.9.10 occasionally sends NaN values in the compass
            compass = 0.0
    ...
    def run_step_transfuser(self, input_data, tick_data):
        ...
        if self.backbone == 'latentTF':  # Image only method
            ...
            if self.config.use_point_pillars:
                ...
                lidar_bev = prepare_lidar(tick_data)
            ...
            for i in range(self.model_count):
                ...
                if self.backbone == 'transFuser':
                    pred_wp, _ = self.nets[i].forward_ego(image, lidar_bev, target_point, target_point_image, velocity,
                                                          num_points=num_points, save_path=SAVE_PATH,
                                                          stuck_detector=self.stuck_detector,
                                                          forced_move=is_stuck, debug=self.config.debug,
                                                          rgb_back=self.rgb_back)
                    ...
                    curr_bev_points, curr_cam_points = lidar_bev_cam_correspondences(deepcopy(tick_data['lidar']),
                                                                                     lidar_bev, image, self.step, False)
                    ...
                    pred_wp, _ = self.nets[i].forward_ego(image, lidar_bev, target_point, target_point_image, velocity,
                                                          bev_points, cam_points, num_points=num_points)
                elif self.backbone == 'latentTF':
                    pred_wp, rotated_bb = self.nets[i].forward_ego(image, lidar_bev, target_point, target_point_image,
                                                                   velocity, num_points=num_points)
                else:
        ...
        self.pred_wp = \
            torch.median(torch.from_numpy(self.pred_wp).to('cuda', dtype=torch.float32), dim=0, keepdims=True)[0]
    ...
    def bb_detected_in_front_of_vehicle(self, ego_speed):
        ...
        # Safety box
        break_distance = ((ego_speed.cpu().numpy().item() * 3.6) / 10.0) ** 2 / 2.0
        # Break distance formula for emergency break
        safety_x = np.clip(break_distance + 1.0, a_min=2.0, a_max=4.0)  # plus one meter is the car.
        ...
        for bb in self.bb_buffer[-1]:
            ...
            if check_obb_intersection(safety_bounding_box, bb_local):
    ...
    def prepare_image(self, tick_data):
        ...
        for degree in self.aug_degrees:
            ...
            rgb = torch.from_numpy(
                shift_x_scale_crop(image, scale=self.config.scale, crop=self.config.img_resolution,
...
class EgoModel:
    def __init__(self, dt=1. / 4):
        ...
        self.throttle_accel = 0.5633837

    def forward(self, locs, yaws, spds, acts):
        # Kinematic bicycle model. Numbers are the tuned parameters from World on Rails
        steer = acts[..., 0:1].item()
        throttle = acts[..., 1:2].item()
        brake = acts[..., 2:3].astype(np.uint8)

        if brake:
            accel = self.brake_accel
        else:
            accel = self.throttle_accel * throttle
